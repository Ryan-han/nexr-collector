...
UFTP_CONF_HOME=$CONF_HOME/uftp
CONF_FILE=$UFTP_CONF_HOME/uftp.conf
...
if [ "$UFTP_PID_DIR" = "" ]; then
  UFTP_PID_DIR=/tmp
fi
log=$UFTP_PID_DIR/uftp.out
pid=$UFTP_PID_DIR/uftp.pid
STOP_TIMEOUT=${STOP_TIMEOUT:-3}

case $actionCmd in

  (start)
    [ -w "$UFTP_PID_DIR" ] ||  mkdir -p "$UFTP_PID_DIR"
    ...
    $FLUME_HOME/bin/flume-ng agent --conf $UFTP_CONF_HOME --conf-file $CONF_FILE -n $AGENT_NAME -Dlog.dir=${LOG_DIR} $MONITOR > "$log" 2>&1 </dev/null &
